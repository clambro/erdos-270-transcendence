...
\hypersetup{
  pdftitle={Transcendence in the Affine Case of Erdos Problem 270},
  pdfsubject={Transcendence for positive affine functions in Erdos Problem 270},
...
Erd\H{o}s and Graham singled out the simplest nondecreasing choice, $f(n)=n$. They described the resulting series as difficult to treat and wrote that such series were surely irrational~\cite[pp.~65--66]{erdos-graham}. Crmari\'c and Kova\v{c} later returned to this example and reported that they knew no proof of its irrationality~\cite[p.~55, following (1.1)]{crmaric-kovac}. After their paper appeared, Crmari\'c and Kova\v{c} found a short proof, which Kova\v{c} posted on the Erd\H{o}s Problems forum in July 2026~\cite{kovac-forum}. We obtained the argument below independently, before learning of their post. The series is the first member of the affine family considered here:

\[
S(f)=C:=\sum_{n=1}^{\infty}\frac{n!}{(2n)!}
=\sum_{n=1}^{\infty}\frac1{D_n},
\qquad
D_n:=\frac{(2n)!}{n!}.
\]
Then
...
R_N:=C-P_N,
\]
then $D_NP_N\in\mathbb Z$, while
\[
\begin{aligned}
0<D_NR_N
&=\sum_{k=1}^{\infty}\frac{D_N}{D_{N+k}}\\
&=\sum_{k=1}^{\infty}
\frac1{(4N+2)(4N+6)\cdots(4N+4k-2)}\\
&\leq\sum_{k=1}^{\infty}\frac1{(4N+2)^k}
=\frac1{4N+1}.
\end{aligned}
\]
Thus $D_NR_N\to0$.

Suppose that $C=p/q$, where $p\in\mathbb Z$ and $q\in\mathbb Z_{>0}$. Then
\[
D_NR_N=\frac{pD_N}{q}-D_NP_N
\]
is positive and belongs to $q^{-1}\mathbb Z$. It must therefore be at least $1/q$, contradicting $D_NR_N\to0$. Hence $C$ is irrational.

\section{Irrationality for \texorpdfstring{$0\leq b\leq a$}{0 <= b <= a}}
\label{sec:irrationality-range}
...
\section{Transcendence of \texorpdfstring{$C_{1,0}$ and $C_{1,1}$}{C(1,0) and C(1,1)}}
\label{sec:a1-transcendence}

The elementary proof above establishes the irrationality of $C_{1,0}$. Its Gaussian integral representation leads to an independent proof of transcendence. Crmari\'c and Kova\v{c} record the identity~\cite[p.~55 and footnote~1]{crmaric-kovac}
\[
...
To pass from this special argument to the full family, we express the case $a=1$ as a hypergeometric $E$-function. This introduces the method used in Section~\ref{sec:affine-family} and treats $C_{1,1}$ as well. For $b\in\{0,1\}$, define
\[
...
\[
F_{1,b}(z)={}_1F_1\!\left(1;\beta;\frac z4\right).
\]
...
\[
zF_{1,b}'(z)+\left(\beta-1-\frac z4\right)F_{1,b}(z)=\beta-1.
\tag{1}
\]
Thus $(1,F_{1,b})^{\mathsf T}$ satisfies a first-order system over $\overline{\mathbb Q}(z)$ whose only finite singularity is $0$.

The functions $1$ and $F_{1,b}$ are linearly independent over $\overline{\mathbb Q}(z)$. Otherwise $F_{1,b}$ would be rational; because it is entire, it would be a polynomial, contrary to its infinitely many nonzero Taylor coefficients. Beukers~\cite[Corollary~1.4]{beukers} proves that $E$-functions which are linearly independent over $\overline{\mathbb Q}(z)$ remain linearly independent after evaluation at a nonzero algebraic point that is ordinary for their differential system. Applied at $z=1$, this gives linear independence of $1$ and $F_{1,b}(1)$ over $\overline{\mathbb Q}$. Hence $F_{1,b}(1)$ is transcendental, and so are both $C_{1,0}$ and $C_{1,1}$. The case $b=0$ recovers the preceding conclusion, while $b=1$ gives the other case in the range $0\leq b\leq a$ when $a=1$.

\section{Transcendence of the affine family}
\label{sec:affine-family}

\begin{theorem}
\label{thm:main}
For all integers $a\geq1$ and $b\geq1-a$, the number $C_{a,b}$ is transcendental.
\end{theorem}
...
Combining this with the cases $1-a\leq b\leq a$ treated above proves Theorem~\ref{thm:main}. In terms of Problem 270, $S(f)$ is transcendental for every positive integer-valued affine function $f(n)=an+b$ with $a\geq1$. Thus the theorem settles the entire affine subclass of the nondecreasing version of the problem.

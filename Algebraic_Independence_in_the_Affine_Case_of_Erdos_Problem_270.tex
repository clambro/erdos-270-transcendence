...
\begin{quote}
\small
Erd\H{o}s Problem 270 concerns the irrationality of a family of rapidly convergent series indexed by positive integer-valued functions $f$ with $f(n)\to\infty$. We treat the affine functions $f(n)=an+b$, for which the series becomes
\[
...
\]
Consequently every $C_{a,b}$ in this range is transcendental.\footnote{An earlier version of this paper established only individual transcendence. Christopher D.~Long pointed out that the same machinery should also yield algebraic independence~\cite{long-x}; the argument was then strengthened to the form given here.}
\end{theorem}
...
The direct-sum decomposition gives a $K$-linear projection onto the zero-exponential summand which commutes with $\theta$. A formal fundamental matrix is generated by Laurent series, formal powers of $z$, and the exponentials $e^{\rho_a\zeta z}$, without adjoining $\log z$. Salikhov's admissible-field construction treats the logarithm as a separate transcendental generator~\cite[pp.~207--210]{salikhov}. Hence the formal Picard--Vessiot field generated by this matrix is closed under field operations and differentiation without acquiring powers of $\log z$. The formal image of $\mathcal K_a$ lies in this log-free field.
...
Apply Lemma~\ref{lem:trace-descent} to~(10), with $D=\theta/a$ and $p=1$. The homogeneous solutions are $cz^{-a}$ with $c\in\mathbb C$, and already belong to $\mathcal K_a$. If $G$ were algebraic over $\mathcal K_a$, the trace lemma would imply $G\in\mathcal K_a$, contrary to Lemma~\ref{lem:field-log-obstruction}. Hence $G$ is transcendental over $\mathcal K_a$, and
\[
